...
`MemberStatus`(ACTIVE / SUSPENDED / WITHDRAWN / DORMANT)는 Domain·Entity·DB 컬럼(`member_status`)으로 **저장·조회만** 가능하다.

| 검색 대상 | 결과 |
|-----------|------|
| `AuthUseCase` / Controller | `member_status`를 갱신하는 API·UseCase 없음 |
| `AuthDomain` | `isActive()` 조회만. `suspend()` 등 상태 변경 메서드 없음 |
| 사용처 | `signIn`·`refresh`에서 `auth.isActive()` 검사 후 `MemberNotActiveException` |

가입 시 기본값 `ACTIVE`(`AuthDomain.createSignUp`). 이후 상태를 바꾸는 **공식 Application 경로는 없음**.

**통합 검증:** member status change → session invalidation 플로우는 구현·E2E 검증 **불가**.

## 3. inactive 계정 refresh 시 `revokeSessionForInactiveAccount()` (`AuthService`)

유일하게 **세션을 적극 revoke**하는 Application 코드는 refresh 실패 분기다.

```
refresh()
  → authRepositoryPort.findByAuthUuid()
  → !auth.isActive()
       → revokeSessionForInactiveAccount(authUuid)
       → throw MemberNotActiveException (403 AUTH_MEMBER_NOT_ACTIVE)
```

`revokeSessionForInactiveAccount(String authUuid)` 동작:
...
> **sign-in**에서 inactive 계정은 `MemberNotActiveException`만 던지고 **revoke 호출 없음** (기존 세션이 Redis에 남을 수 있음). inactive 세션 정리는 **refresh 시도 시**에만 발생.
...
## 6. `SessionInvalidationService` — 미구현

공통 세션 무효화 전용 Application 서비스(`SessionInvalidationService` 등)는 **코드베이스에 없음**.

현재 revoke 로직은 `AuthService` private `revokeSessionForInactiveAccount()` 및 `logout()` / sign-in 시 `issueTokens(..., invalidatePreviousSession=true)`에 **분산**.

비밀번호 변경·회원 상태 변경 UseCase가 추가될 때 **동일 Redis 작업을 한 곳에서 호출**하도록 공통화하는 것이 권장된다 (아래 §7).
...
| Member status change (SUSPENDED, WITHDRAWN, DORMANT 등) | inactive 계정의 기존 세션 즉시 종료 |

권장 순서 (개념):

1. `@Transactional` 내 auth Domain/DB 갱신
2. **공통 invalidation** — 활성 access jti → blacklist, `auth:access:*`·`auth:refresh:*` 삭제 (`revokeSessionForInactiveAccount`와 동일 Redis 계약)
3. (선택) Gateway blacklist TTL은 기존과 동일 정책 (`auth-cookie-flow.md` sign-in blacklist 참고)

`logout()`·inactive refresh revoke와 **동일 Redis 키·TTL 규칙**을 재사용해 drift를 막는다.
...
`AuthService.revokeSessionForInactiveAccount` 주석과 동일:
...
**요약:** password change / member status change invalidation은 **미구현**. 실제 revoke는 **logout**, **sign-in(이전 access blacklist)**, **inactive refresh**(`revokeSessionForInactiveAccount`)에 한정. 향후 변경 UseCase는 공통 session invalidation 호출이 **필수**이며, auth DB 직접 UPDATE는 세션 일관성을 깨므로 금지.
